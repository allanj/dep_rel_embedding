...
class Config:
    ...
    def read_pretrain_embedding(self):
        ...
        if self.embedding_file is None or (not os.path.isfile(self.embedding_file)):
    ...
    def build_deplabel_idx(self, insts):
        if self.mask_label not in self.deplabel2idx:
            self.deplabels.append(self.mask_label)
            self.deplabel2idx[self.mask_label] = len(self.deplabel2idx)
            self.mask_label_id = self.deplabel2idx[self.mask_label]
        for inst in insts:
