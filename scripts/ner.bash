...
context_emb=none
num_epochs_all=(100)
devices=(cuda:0)   ##cpu, cuda:0, cuda:1
...
freezes=(1)
dep_emb_size=200
pdep_model=ontonotes_500_levi_1
...
for (( d=0; d<${#datasets[@]}; d++  )) do
    ...
    python3.6 main.py --context_emb ${context_emb}  --train_num ${train_num}\
      --dataset ${dataset}  --num_epochs ${num_epochs} --device ${device}  --num_lstm_layer ${num_lstm_layer} \
       ...
       --embedding_file ${emb} --inter_func ${inter_func} --dep_emb_size ${dep_emb_size} \
       --pdep_model ${pdep_model} > ${logfile} 2>&1
